...
describe("ast node", () => {
  describe("childNodeRange", () => {
    ...
    test("expression", () => {
      ...
      expect(node.childNodeRange("expression.callee.object")).toEqual({
        start: 0,
        end: 3,
      });
      expect(node.childNodeRange("expression.callee.dot")).toEqual({
        start: 3,
        end: 4,
      });
      expect(node.childNodeRange("expression.callee.property")).toEqual({
        start: 4,
        end: 12,
      });
      expect(node.childNodeRange("expression.arguments")).toEqual({
        start: 12,
        end: 14,
      });
    });
    ...
    test("expression with arguments index", () => {
      ...
      expect(node.childNodeRange("expression.arguments.0")).toEqual({
        start: 5,
        end: 8,
      });
    });
    ...
    test("expression with first element", () => {
      ...
      expect(node.childNodeRange("expression.arguments.first")).toEqual({
        start: 5,
        end: 8,
      });
    });
    ...
    test("method definition", () => {
      ...
      `);
      expect(node.childNodeRange("body.body.0.async")).toEqual({
        start: 34,
        end: 40,
      });
      expect(node.childNodeRange("body.body.0.value.params")).toEqual({
        start: 46,
        end: 56,
      });
    });
    ...
    test("property", () => {
      ...
      const node = parse(code);
      expect(
        node.childNodeRange("declarations.0.init.properties.0.semicolon")
  ...
  describe("match", () => {
    test("matches class id", () => {
      ...
      const node = parse(code);
      expect(node.match({ type: "ClassDeclaration", id: "Synvert" })).toBe(
        true
      );
    });
    ...
    test("matches class id with name", () => {
      ...
      const node = parse(code);
      expect(
        node.match({ type: "ClassDeclaration", id: { name: "Synvert" } })
      ).toBe(true);
    });
    ...
    test("matches regexp with toSource", () => {
      ...
      mock({ "code.js": code });
      const node = parse(code).expression;
      expect(node.match({ type: "MemberExpression", object: /foo/ })).toBe(
        true
      );
      mock.restore();
    });
    ...
    test("matches not", () => {
      ...
      const node = parse(code);
      expect(
        node.match({ type: "ClassDeclaration", id: { not: "FooBar" } })
      ).toBe(true);
    });
    ...
    test("matches notIn", () => {
      ...
      const node = parse(code);
      expect(
        node.match({ type: "ClassDeclaration", id: { notIn: ["Foo", "Bar"] } })
      ).toBe(true);
    });
  ...
  describe("rewrittenSource", () => {
    ...
    test("rewrites for arguments", () => {
      ...
      mock({ "code.js": code });
      const node = parse(code);
      expect(node.rewrittenSource("{{expression.arguments}}")).toBe(
        "'foo', 'bar'"
      );
      mock.restore();
    });

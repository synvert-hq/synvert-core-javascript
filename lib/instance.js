...
class Instance {
   ...
   */
  ifAllNodes(rules, options, function_, elseFunction) {
    new IfAllCondition(Instance.current, rules, options, function_, elseFunction).process();
  }
   ...
   */
  replaceWith(code) {
    Instance.current.actions.push(new ReplaceWithAction(Instance.current, code).process());
  }

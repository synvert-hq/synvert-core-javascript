...
const { IfExistCondition, UnlessExistCondition, IfOnlyExistCondition } = require("./condition");
const {
  AppendAction,
  PrependAction,
  InsertAction,
...
class Instance {
   ...
   */
  constructor(rewriter, filePattern, function_) {
   ...
   */
  process() {
    glob.sync(this.filePattern, { ignore: Configuration.skipFiles, cwd: Configuration.path, nodir: true, realpath: true, absolute: true }).forEach((filePath) => {
      while (true) {
   ...
   * @param {Function} func
   */
  processWithNode(node, function_) {
   ...
   * @param {Function} func
   */
  processWithOtherNode(node, function_) {
   ...
   */
  withinNode(rules, function_) {
    new WithinScope(Instance.current, rules, function_).process();
  }
   ...
   */
  gotoNode(childNodeName, function_) {
    new GotoScope(Instance.current, childNodeName, function_).process();
  }
   ...
   * @param {Function} func - to continue operating on the matching nodes.
   */
  ifExistNode(rules, function_) {
    new IfExistCondition(Instance.current, rules, function_).process();
  }
   ...
   * @param {Function} func - to continue operating on the matching nodes.
   */
  unlessExistNode(rules, function_) {
    new UnlessExistCondition(Instance.current, rules, function_).process();
  }
   ...
   * @param {Function} func - to continue operating on the matching nodes.
   */
  ifOnlyExistNode(rules, function_) {
    new IfOnlyExistCondition(Instance.current, rules, function_).process();
  }
   ...
   */
  append(code) {
    Instance.current.actions.push(new AppendAction(Instance.current, code));
  }
   ...
   */
  prepend(code) {
    Instance.current.actions.push(new PrependAction(Instance.current, code));
  }
   ...
   */
  remove(selectors) {
    Instance.current.actions.push(new RemoveAction(Instance.current));
  }
  ...
  _getConflictActions() {
    ...
    let beginPos = this.actions[index].beginPos();
    while (index_ > -1) {
      if (beginPos < this.actions[index_].endPos()) {
        conflictActions.push(this.actions.splice(index_, 1));
      } else {
        index = index_;
        beginPos = this.actions[index].beginPos();
      }
      index_--;
    }

...
class Instance {
   ...
   * It creates a {IfExistCondition} to check if matching nodes exist in the child nodes,
   ...
   * It creates a {UnlessExistCondition} to check if matching nodes does not exist in the child nodes,
   * if so, then continue operating on each matching ast node.
   *
   * @param {Object} rules - to check mathing ast nodes.
   * @param {Object} options - to do find in specific child node, e.g. { in: 'callee' }
   ...
   * It creates a {CommentOutAction} to comment out current node.
   */
  commentOut() {

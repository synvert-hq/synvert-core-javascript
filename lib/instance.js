...
const { indent, dedent } = require("./utils");
...
class Instance {
   ...
   */
  process() {
    if (fs.existsSync(this.filePattern)) {
      return this._processFile(this.filePattern);
    }
  ...
  _processFile(filePath) {
    this._currentFilePath = path.relative(Configuration.path, filePath);

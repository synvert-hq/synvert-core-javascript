...
class Rewriter {
  ...
  addSnippet(group, name) {
    Rewriter.current._subSnippets.push(Rewriter.call(group, name))
  }
  ...
  withinFiles(filePattern, function_) {
    const instance = new Instance(this, filePattern, function_);

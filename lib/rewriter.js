...
class Rewriter {
   ...
   */
  constructor(group, name, function_) {
    this.group = group;
    this.name = name;
    this.subSnippets = [];
    this.func = function_;
  ...
  addSnippet(group, name) {
    Rewriter.current.subSnippets.push(Rewriter.call(group, name));
  }
   ...
   */
  withinFiles(filePattern, function_) {
    if ((!Rewriter.current.nodeVersion || Rewriter.current.nodeVersion.match()) && (!Rewriter.current.npmVersion || Rewriter.current.npmVersion.match())) {
      const instance = new Instance(Rewriter.current, filePattern, function_);

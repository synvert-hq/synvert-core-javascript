...
class Action {
  ...
  _removeComma() {
    if ("," === this._source()[this._beginPos - 1]) {
      this._beginPos = this._beginPos - 1
    } else if (", " === this._source().slice(this._beginPos - 2, this._beginPos)) {
      this._beginPos = this._beginPos - 2
    } else if (", " === this._source().slice(this._endPos, this._endPos + 2)) {
      this._endPos = this._endPos + 2
    } else if ("," === this._source()[this._endPos + 1]) {
...
class ReplaceWithAction extends Action {
  ...
  get rewrittenCode() {
    if (this._rewrittenSource().includes("\n")) {
      const newCode = [];
      for (const [index, line] of this._rewrittenSource()
        .split("\n").entries()) {
          if (index === 0) {
            newCode.push(line);
          } else {

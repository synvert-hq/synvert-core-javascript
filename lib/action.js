...
class AppendAction extends Action {
  ...
  rewrittenCode() {
    ...
    const indent = this.node.type == "Program" ? "" : " ".repeat(this.node.column() + 2);
    if (source.split("\n").length > 1) {
      return (
        source
          .split("\n")
...
class PrependAction extends Action {
   ...
   */
  beginPos() {
    return this.node.body.start + this.node.column() + "{\n".length;
  }
...
class RemoveAction extends Action {
  ...
  endPos() {
    if (this._takeWholeLine()) {
      return this._source().split("\n").slice(0, this.node.loc.end.line).join("\n").length + (this.node.loc.end.line === 1 ? 0 : "\n".length);
    } else {
      return this.node.end;
    }
  }
  ...
  _takeWholeLine() {
    const sourceFromFile = this._source().split("\n").slice(this.node.loc.start.line - 1, this.node.loc.end.line).join("\n").trim()
...
class ReplaceWithAction extends Action {
  ...
  rewrittenCode() {
    if (this.rewrittenSource().includes("\n")) {
      const newCode = [];
      for (const [index, line] of this.rewrittenSource()
        .split("\n").entries()) {
          if (index === 0) {
            newCode.push(line);
          } else {

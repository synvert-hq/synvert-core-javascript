...
np.childNodeRange = function(childName) {
  if (this.type === 'MemberExpression' && childName === 'dot') {
...
np.match = function(rules) {
  return Object.keys(flatten(rules)).every(multi_key => {
    const actual = t(this, multi_key).safeObject;
...
np.rewrittenSource = function(code) {
  return code.replace(/{{(.*?)}}/mg, (string, match, offset) => {
    if (match && t(this, match).isDefined) {
      const obj = t(this, match).safeObject
...
np._matchValue = function(actual, expected) {
  return actual === expected || actual.name === expected || (actual.type === 'ThisExpression' && expected === 'this');
}

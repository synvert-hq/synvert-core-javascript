 ...
 */
np.recursiveChildren = function (function_) {
  visitorKeys.KEYS[this.type].forEach((key) => {
    if (Array.isArray(this[key])) {
      this[key].forEach((child) => {
        if (child) {
          function_(child);
          child.recursiveChildren(function_);
        }
      });
    } else {
...
np.match = function (rules) {
  let actual, expected, expectedValues;
  return Object.keys(flatten(rules, { safe: true })).every((multiKey) => {
    ...
    switch (lastKey) {
      ...
      case "in":
        actual = this.actualValue(keys.slice(0, -1));
        expectedValues = t(rules, multiKey).safeObject;
        return expectedValues.some(expected => this._matchValue(actual, expected));
      case "notIn":
        actual = this.actualValue(keys.slice(0, -1));
        expectedValues = t(rules, multiKey).safeObject;
        return expectedValues.every(expected => !this._matchValue(actual, expected));
    default:
...
np.rewrittenSource = function (code) {
  return code.replace(/{{(.*?)}}/gm, (_string, match, _offset) => {
    ...
    const object = this.actualValue(match.split("."));
    if (object) {
      if (Array.isArray(object)) {
        return this._fileContent().slice(object[0].start, object[object.length - 1].end);
      }
      const result = object.hasOwnProperty("name") ? object.name : object;
